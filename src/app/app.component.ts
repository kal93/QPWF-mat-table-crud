  ...
  addNew(authorsData: AuthorsInterface) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === 1) {
        ...
        this.exampleDatabase.dataChange.value.push(this.tableAuthorsService.getDialogData());
        this.refreshTable();
      }
    });
  }
  ...
  startEdit(i: number, id: number, title: string, state: string, url: string, created_at: string, updated_at: string) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === 1) {
        ...
        this.exampleDatabase.dataChange.value[foundIndex] = this.tableAuthorsService.getDialogData();
...
export class ExampleDataSource extends DataSource<AuthorsInterface> {
  ...
  constructor(public _exampleDatabase: TableAuthorsService,
  ...
  connect(): Observable<AuthorsInterface[]> {
    ...
    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.data.slice().filter((authorsData: AuthorsInterface) => {
        const searchStr = (authorsData.id + authorsData.title + authorsData.url + authorsData.created_at).toLowerCase();
